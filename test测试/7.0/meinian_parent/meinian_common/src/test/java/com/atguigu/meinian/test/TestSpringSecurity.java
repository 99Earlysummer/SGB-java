package com.atguigu.meinian.test;

import org.junit.Test;
import org.springframework.security.crypto.bcrypt.BCryptPasswordEncoder;

public class TestSpringSecurity {
    // SpringSecurity加盐-密
    @Test
    public void testSpringSecurity(){
        // $2a$10$dyIf5fOjCRZs/pYXiBYy8uOiTa1z7I.mpqWlK5B/0icpAKijKCgxe
        // $2a$10$OphM.agzJ55McriN2BzCFeoLZh/z8uL.8dcGx.VCnjLq01vav7qEm

        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        /*String s = encoder.encode("abc");
        System.out.println(s);

        String s1 = encoder.encode("123456");
        System.out.println(s1);*/

        // 进行判断 abc<-->2a$10$dyIf5fOjCRZs/pYXiBYy8uOiTa1z7I.mpqWlK5B/0icpAKijKCgxe
        boolean b = encoder.matches("123456", "$2a$10$lLX9iRNjccKVBS.QrCWBOOxDcEyndKA7wrT/IKGiGrHZj3Zx4ybfu");
        System.out.println(b);
    }
}

